...
class StoreExecutorImpl implements StoreExecutor {
  ...
  Zone get _executionZone => Zone.current.fork(
  ...
  RunHandler get _run => <R>(source, parent, zone, fn) {
  ...
  RunUnaryHandler get _runUnary => <R, X>(source, parent, zone, fn, x) {
        if (_controller.isTeardowned) return null;
        if (_controller.hasTransanction) {
  ...
  RunBinaryHandler get _runBinary => <R, X, Y>(source, parent, zone, fn, x, y) {
        if (_controller.isTeardowned) return null;
        if (_controller.hasTransanction) {
